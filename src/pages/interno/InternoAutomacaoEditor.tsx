...
const NODE_COLOR: Record<FlowNode['type'], string> = { dm: '#FFE14D', public_reply: '#16A34A', link: '#2563EB', delay: '#B45309', reminder: '#B45309', ask: '#C026D3', condition: '#374151', tag: '#6B7280' };
    ...
    <div className="-m-4 lg:-m-6 flex h-[calc(100vh-48px)] min-h-[600px] flex-col overflow-hidden bg-background">
      ...
      <div className="flex flex-wrap items-center gap-3 border-b border-border px-4 py-3 lg:px-6">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {auto.status === 'active' ? <Button variant="outline" size="sm" onClick={() => void save('paused')} disabled={saving}><Power size={14} className="mr-1.5" />Pausar</Button> : <Button size="sm" onClick={() => void save('active')} disabled={saving} className="bg-[#FFE14D] text-black hover:bg-[#E5C62E]"><Zap size={14} className="mr-1.5" />Publicar</Button>}
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[280px_1fr] xl:grid-cols-[280px_1fr_380px]">
        ...
        <aside className="flex min-h-0 flex-col gap-4 overflow-y-auto border-r border-border bg-card px-4 py-4">
          ...
          <div className="relative flex flex-col">
            ...
            <div className="relative"><button onClick={(e) => { e.stopPropagation(); setMenuAdd((o) => !o); }} className="flex w-full items-center gap-3 rounded-lg px-2 py-2 text-left text-[#E5C62E] hover:bg-muted/50"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-full border-2 border-dashed border-[#FFE14D]"><Plus size={12} /></span><span className="text-sm font-semibold">Adicionar etapa</span></button>{menuAdd && <MenuEtapas onAdd={addNode} className="left-8 top-10" />}</div>
          </div>
          ...
          <div className="mt-auto flex gap-2.5 rounded-lg bg-muted px-3 py-2.5 text-xs text-muted-foreground"><Sparkles size={14} className="shrink-0 text-[#E5C62E]" /><span><strong className="text-foreground">Sugestão:</strong> {sugestao}</span></div>
        </aside>
        ...
        <div className="relative min-h-0 overflow-auto bg-muted/30 [background-image:radial-gradient(hsl(var(--border))_1px,transparent_1px)] [background-size:22px_22px]">
          <div className="flex min-h-full flex-col items-center px-6 pb-24 pt-10">
            <button onClick={() => setSelected('trigger')} className="w-[340px] rounded-xl border-2 bg-card px-4 py-3.5 text-left shadow-sm" style={{ borderColor: selected === 'trigger' ? '#FFE14D' : '#22C55E' }}>
            ...
            {def.nodes.map((n, i) => { const Icon = NODE_ICON[n.type]; const texto = summary(n); const on = selected === n.id; return (
              <div key={n.id} className="flex flex-col items-center">
                <div className="flex h-12 items-center"><svg width="24" height="48" viewBox="0 0 24 48"><path d="M12 2 L12 42" fill="none" stroke="#FFE14D" strokeWidth="2" strokeDasharray="5 4" /><path d="M6 36 L12 42 L18 36" fill="none" stroke="#FFE14D" strokeWidth="2" strokeLinecap="round" /></svg></div>
                <button onClick={() => setSelected(n.id)} className="w-[340px] rounded-xl border-2 bg-card px-4 py-3.5 text-left shadow-sm" style={{ borderColor: on ? '#FFE14D' : 'transparent' }}>
            ...
            <div className="relative"><button onClick={(e) => { e.stopPropagation(); setMenuAdd((o) => !o); }} className="flex h-10 items-center gap-2 rounded-full border-2 border-dashed border-[#FFE14D] bg-card px-4 text-xs font-semibold text-[#E5C62E] hover:bg-muted"><Plus size={14} />Adicionar etapa</button>{menuAdd && <MenuEtapas onAdd={addNode} className="left-1/2 top-12 -translate-x-1/2" />}</div>
          </div>
        </div>
